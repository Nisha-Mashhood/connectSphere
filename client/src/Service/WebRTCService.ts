export class WebRTCService {
  ...
  private onTrackCallbacks: Map<
    string,
    (event: RTCTrackEvent, targetId: string) => void
  > = new Map();
  private onIceCandidateCallbacks: Map<
    string,
  ...
  async initPeerConnection(
    ...
    if (!targetId) {
      if (
        ...
        this.singlePeerConnection.connectionState !== "closed"
      ) {
        console.log(
          "Single peer connection already initialized, returning existing"
        );
      ...
      if (
        this.peerConnections.has(targetId) &&
        this.peerConnections.get(targetId)!.connectionState !== "closed"
      ) {
        console.log(
          `Peer connection for ${targetId} already initialized, returning existing`
        );
      ...
      console.log(
        `Initializing peer connection for ${targetId} with config:`,
        config
      );
      try {
        ...
        console.error(
          `Error initializing peer connection for ${targetId}:`,
          error
        );
        throw error;
      }
  ...
  private setupPeerConnectionEvents(
    ...
    peerConnection.oniceconnectionstatechange = () => {
      console.log(
        `ICE connection state for ${targetId}: ${peerConnection.iceConnectionState}`
      );
    ...
    peerConnection.onconnectionstatechange = () => {
      console.log(
        `Connection state for ${targetId}: ${peerConnection.connectionState}`
      );
      if (peerConnection.connectionState === "connected") {
        console.log(`Peer connection established for ${targetId}`);
      } else if (peerConnection.connectionState === "failed") {
        console.error(
          `Connection failed for ${targetId}, attempting to restart ICE`
        );
        try {
          peerConnection.restartIce();
        } catch (error) {
    ...
    peerConnection.onsignalingstatechange = () => {
      console.log(
        `Signaling state for ${targetId}: ${peerConnection.signalingState}`
      );
    };

    peerConnection.ontrack = (event) => {
      console.log(
        `Received remote track from ${targetId}, streams:`,
        event.streams
      );
      if (event.streams[0]) {
        const tracks = event.streams[0].getTracks();
        console.log(
          `Remote stream tracks for ${targetId}:`,
          tracks.map((t) => ({
            kind: t.kind,
            id: t.id,
            enabled: t.enabled,
    ...
    peerConnection.onicecandidate = (event) => {
      if (event.candidate) {
        console.log(
          `Generated ICE candidate for ${targetId}:`,
  ...
  hasRemoteDescription(targetId?: string): boolean {
    if (!targetId) {
      ...
      console.log(
        `Remote description for single: ${hasRemoteDesc}, signalingState: ${this.singlePeerConnection.signalingState}`
      );
    ...
    console.log(
      `Remote description for ${targetId}: ${hasRemoteDesc}, signalingState: ${peerConnection.signalingState}`
    );
    return hasRemoteDesc;
  }
  ...
  async createOffer(targetId?: string): Promise<RTCSessionDescriptionInit> {
    const peerConnection = targetId
      ? this.peerConnections.get(targetId)
      : this.singlePeerConnection;
    if (!peerConnection)
      throw new Error(
        `Peer connection not initialized for ${targetId || "single"}`
      );
    try {
      const offer = await peerConnection.createOffer();
      await peerConnection.setLocalDescription(offer);
      console.log(
        `Offer created and local description set for ${targetId || "single"}:`,
        offer.sdp
      );
      console.log(
        ` Transceivers before offer for ${targetId}:`,
  ...
  async createAnswer(targetId?: string): Promise<RTCSessionDescriptionInit> {
    ...
    if (!this.hasRemoteDescription(targetId)) {
      console.warn(
        `No remote description set for ${
          targetId || "single"
        } before creating answer`
      );
      throw new Error(`No remote description set for ${targetId || "single"}`);
    }
    try {
      const senders = peerConnection.getSenders().filter((s) => s.track);
      if (!senders.length) {
        console.warn(
          ` No local tracks found before createAnswer for ${
            targetId || "single"
          }`
        );
      } else {
        console.log(
          `Local tracks before answer for ${targetId || "single"}:`,
          senders.map((s) => s.track!.kind)
        );
      }
      const transceivers = peerConnection.getTransceivers();
      console.log(
        `[DEBUG] Transceivers before creating answer for ${
          targetId || "single"
        }:`,
        transceivers.map((t) => ({
          mid: t.mid,
      ...
      transceivers.forEach((transceiver) => {
        if (transceiver.sender.track && transceiver.direction !== "sendrecv") {
          console.log(
            `[DEBUG] Setting transceiver direction to sendrecv for ${transceiver.sender.track.kind} track: ${transceiver.sender.track.id}`
          );
      ...
      console.error(
        `Error creating answer for ${targetId || "single"}:`,
        error
      );
      throw error;
    }
  ...
  async setRemoteDescription(
    descriptionOrTargetId: string | RTCSessionDescriptionInit,
    ...
    if (typeof descriptionOrTargetId === "string") {
      ...
      if (!peerConnection)
        throw new Error(
          `Peer connection not initialized for ${descriptionOrTargetId}`
        );
      if (!description)
        throw new Error("Description is required for group calls");
      if (
        description.type === "answer" &&
        peerConnection.signalingState === "stable"
      ) {
        console.log(
          `Ignoring duplicate answer for ${descriptionOrTargetId} in stable state`
        );
        return;
      }
      console.log(
        `Setting remote description for ${descriptionOrTargetId}:`,
        description
      );
      try {
        ...
        console.log(
          `Remote description set successfully for ${descriptionOrTargetId}, signalingState:`,
          peerConnection.signalingState
        );
        // Process queued ICE candidates
        const queuedCandidates = this.pendingIceCandidates.get(
          descriptionOrTargetId
        );
        if (queuedCandidates) {
          for (const candidate of queuedCandidates) {
            console.log(
              `Processing queued ICE candidate for ${descriptionOrTargetId}:`,
              candidate
            );
            ...
            console.log(
              `Queued ICE candidate added successfully for ${descriptionOrTargetId}`
            );
          }
        ...
        console.error(
          `Error setting remote description for ${descriptionOrTargetId}:`,
          error
        );
        throw error;
      }
      ...
      if (!this.singlePeerConnection)
        throw new Error("Single peer connection not initialized");
      if (
        descriptionOrTargetId.type === "answer" &&
        this.singlePeerConnection.signalingState === "stable"
      ) {
        console.log(
          `Ignoring duplicate answer for single peer connection in stable state`
        );
        return;
      }
      console.log(
        "Setting remote description for single peer connection:",
        descriptionOrTargetId
      );
      try {
        await this.singlePeerConnection.setRemoteDescription(
          new RTCSessionDescription(descriptionOrTargetId)
        );
        console.log(
          `Remote description set successfully for single, signalingState:`,
          this.singlePeerConnection.signalingState
        );
        // Process queued ICE candidates
        ...
        if (queuedCandidates) {
          for (const candidate of queuedCandidates) {
            console.log(
              `Processing queued ICE candidate for single:`,
              candidate
            );
            await this.singlePeerConnection.addIceCandidate(
              new RTCIceCandidate(candidate)
            );
        ...
        console.error(
          "Error setting remote description for single peer connection:",
          error
        );
        throw error;
      }
  ...
  async addIceCandidate(
    candidateOrTargetId: string | RTCIceCandidateInit,
    ...
    if (typeof candidateOrTargetId === "string") {
      ...
      if (!peerConnection)
        throw new Error(
          `Peer connection not initialized for ${candidateOrTargetId}`
        );
      if (!candidate) throw new Error("Candidate is required for group calls");
      if (!peerConnection.remoteDescription) {
        console.log(
          `Queueing ICE candidate for ${candidateOrTargetId}, no remote description yet`
        );
      ...
      console.log(
        `Adding ICE candidate for ${candidateOrTargetId}:`,
        candidate
      );
      try {
        await peerConnection.addIceCandidate(new RTCIceCandidate(candidate));
        console.log(
          `ICE candidate added successfully for ${candidateOrTargetId}`
        );
      } catch (error) {
        console.error(
          `Error adding ICE candidate for ${candidateOrTargetId}:`,
          error
        );
        throw error;
      }
      ...
      if (!this.singlePeerConnection.remoteDescription) {
        console.log(
          `Queueing ICE candidate for single, no remote description yet`
        );
      ...
      console.log(
        "Adding ICE candidate for single peer connection:",
        candidateOrTargetId
      );
      try {
        await this.singlePeerConnection.addIceCandidate(
          new RTCIceCandidate(candidateOrTargetId)
        );
        console.log("ICE candidate added successfully for single");
      } catch (error) {
        console.error(
          "Error adding ICE candidate for single peer connection:",
          error
        );
        throw error;
      }
  ...
  closePeerConnection(targetId: string): void {
    ...
    try {
      ...
      console.log(
        `Closed peer connection for ${targetId}, connectionState: ${peerConnection.connectionState}`
      );
