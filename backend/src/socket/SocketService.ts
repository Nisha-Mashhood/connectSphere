...
} from "../Modules/Notification/Service/NotificationService";
...
import {
  ...
  Message,
  TypingData,
} from "../Interfaces/Types/ISocketService";
...
export class SocketService {
  ...
  private offerSentSet: Map<string, { userId: string; recipientId: string; timestamp: number }[]> = new Map();
  private sentAnswers = new Map();
  ...
  public initialize(io: Server): void {
    ...
    SocketService.notificationEmitter.on(
      "notification",
  ...
  private handleConnection(socket: Socket): void {
    ...
    logger.info(
      `New client connected: socketId=${
        socket.id
      }, userId=${userId}, auth=${JSON.stringify(socket.handshake.auth)}`
    );

    socket.on("joinChats", (userId: string) =>
      this.handleJoinChats(socket, userId)
    );
    ...
    socket.on("groupIceCandidate", (data: GroupIceCandidateData) =>
      this.handleGroupIceCandidate(socket, data)
    );
    socket.on(
      "groupOffer",
      (data: {
        ...
        callId: string;
      }) => this.handleGroupOffer(socket, data)
    );
    socket.on(
      "groupAnswer",
      (data: {
        ...
        callType: "audio" | "video";
        callId: string;
      }) => this.handleGroupAnswer(socket, data)
    );
    socket.on(
      "groupCallEnded",
      (data: {
        ...
        callType: "audio" | "video";
        callId: string;
      }) => this.handleGroupCallEnded(socket, data)
    );
    socket.on(
      "joinGroupCall",
      (data: {
  ...
  private async handleGroupIceCandidate(
    ...
    try {
      const { groupId, senderId, recipientId, candidate, callType, callId } =
        data;
      logger.info(
        `Received group ${callType} ICE candidate from ${senderId} to ${recipientId} for group ${groupId}, callId: ${callId}`
      );
      ...
      const isSenderMember = await this.groupRepo.isUserInGroup(
        groupId,
        senderId
      );
      const isRecipientMember = await this.groupRepo.isUserInGroup(
        groupId,
        recipientId
      );
      if (!isSenderMember || !isRecipientMember) {
        logger.error(
          `Invalid membership: senderId=${senderId}, recipientId=${recipientId}, groupId=${groupId}`
        );
  ...
  logger.info(
      `Received group ${callType} offer from ${senderId} to ${recipientId} for group ${groupId}, callId: ${callId}`
    );
    logger.info(`Offer got is ${offer}`);
    ...
    // Deduplicate group offers
    const offerKey = `${callId}:${senderId}:${recipientId}`;
    if (!this.offerSentSet.has(callId)) {
      this.offerSentSet.set(callId, []);
    }
    const offerSet = this.offerSentSet.get(callId)!;
    if (offerSet.some((entry) => entry.userId === senderId && entry.recipientId === recipientId)) {
      logger.info(`Ignoring duplicate group offer: ${offerKey}`);
      return;
    }
    offerSet.push({ userId: senderId, recipientId, timestamp: Date.now() });
    ...
    // Cleanup offerSentSet after 30 seconds
    setTimeout(() => {
      const updatedSet = offerSet.filter((entry) => Date.now() - entry.timestamp < 30000);
      if (updatedSet.length === 0) {
        this.offerSentSet.delete(callId);
        logger.info(`Cleaned up offerSentSet for callId: ${callId}`);
      } else {
        this.offerSentSet.set(callId, updatedSet);
      }
    }, 30000);
  } catch (error: any) {
...
private async handleGroupAnswer(
 ...
 const answerKey = `${callId}:${senderId}:${recipientId}`;
    logger.info(
      `Received group ${callType} answer from ${senderId} to ${recipientId} for group ${groupId}, callId: ${callId}`
    );
    logger.info(`Answer got is ${answer}`);
    ...
    if (this.sentAnswers.has(answerKey)) {
      logger.info(`Ignoring duplicate group answer: ${answerKey}`);
      return;
    }

    // Relay answer to recipient
    socket.to(`user_${recipientId}`).emit("groupAnswer", data);
    this.sentAnswers.set(answerKey, Date.now());
    logger.info(`Relayed group answer to user_${recipientId}`);

    // Clean up sentAnswers after 30 seconds
    setTimeout(() => {
      this.sentAnswers.delete(answerKey);
      logger.info(`Cleaned up sentAnswers entry for ${answerKey}`);
    }, 30000);
  } catch (error: any) {
  ...
  private async handleGroupCallEnded(
    socket: Socket,
    data: {
      groupId: string;
      senderId: string;
    ...
    const eventKey = `${groupId}_${callId}_${callType}`;
      ...
      logger.info(
        `Received groupCallEnded from ${senderId} to ${recipientId} for group ${groupId}, callId: ${callId}, callType: ${callType}`
      );
      ...
      const isSenderMember = group.members.some((member) =>
        member.userId.equals(senderObjectId)
      );
      const isRecipientMember = group.members.some((member) =>
        member.userId.equals(recipientObjectId)
      );
      if (!isSenderMember || !isRecipientMember) {
        logger.error(
          `Invalid members: sender ${senderId} or recipient ${recipientId} not in group ${groupId}`
        );
      ...
      // Emit groupCallEnded to all group members
      ...
      for (const memberId of groupMembers) {
        ...
        logger.info(
          `Emitted groupCallEnded to ${recipientRoom} for callId: ${callId}`
        );
      }

      // Clean up offerSentSet for this call
      this.offerSentSet.delete(callId);
      logger.info(`Cleaned up offerSentSet for callId: ${callId}`);

      // Clean up endedCalls after 1 hour
      setTimeout(() => {
      ...
      logger.error(
        `Error handling groupCallEnded for group ${data.groupId}, callId: ${data.callId}:`,
        error
      );
  ...
  socket: Socket,
  data: { groupId: string; userId: string; callType: "audio" | "video"; callId: string }
): Promise<void> {
  const { groupId, userId, callType, callId } = data;
  logger.info(`Received joinGroupCall: user ${userId}, group ${groupId}, callId: ${callId}, callType: ${callType}`);
  socket.join(`groupCall_${groupId}_${callId}`);
  logger.info(`User ${userId} joined group call room: groupCall_${groupId}_${callId}`);
  ...
  const isMember = await this.groupRepo.isUserInGroup(groupId, userId);
  if (!isMember) {
    logger.error(`User ${userId} not in group ${groupId}`);
    socket.emit("error", { message: "Invalid group membership" });
    return;
  }

  // Check offerSentSet and notify client to send offers and/or answers
  const offerSet = this.offerSentSet.get(callId) || [];
  const groupMembers = group.members
  ...
  const remainingMembers = groupMembers.filter(
    (memberId) => !offerSet.some((entry) => entry.userId === memberId && entry.recipientId === userId)
  );
  const answerRecipients = offerSet
    .filter((entry) => entry.recipientId === userId)
  ...
  const payload = {
    ...
    callId,
    callType,
    offerRecipients: remainingMembers,
    answerRecipients,
  };
  logger.info(`Emitting sendGroupOffersAndAnswers with payload: ${JSON.stringify(payload, null, 2)}`);

  // Emit sendGroupOffersAndAnswers
  socket.emit("sendGroupOffersAndAnswers", payload);
  logger.info(`Notified ${userId} to send offers to ${remainingMembers.join(", ")} and answers to ${answerRecipients.join(", ")}`);
}
  ...
  private handleDisconnect(socket: Socket): void {
    logger.info(
      `User disconnected: socketId=${socket.id}, userId=${socket.data.userId}`
    );
  }
