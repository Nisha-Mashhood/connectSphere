        ...
        socket.on("markAsRead", async (data) => {
            try {
                ...
                for (const notification of messageNotifications) {
                    const updatedNotification = await markNotificationAsRead(notification._id.toString());
                ...
                const endTimeout = setTimeout(async () => {
                    ...
                    for (const recipientId of recipientIds) {
                        if (!connectedUserIds.has(recipientId)) {
                            const notification = await updateCallNotificationToMissed(recipientId, callId, `Missed ${callType} call from ${userId}`);
                            if (notification) {
